...
const LikeComment = ({ postObj, userObj, onDetailClick }) => {
  ...
  const onHeartClick = () => {
    isLike
      ? axiosInstance.delete(`/like/${userObj.id}/${postObj.id}`).then(() => {
          setLikeCount((p) => p - 1);
          setIsLike(false);
        })
      : axiosInstance
      ...
      {onDetailClick === undefined ? (
        <div className="heart-comment-wrapper">
          <FontAwesomeIcon
            id="icon"
            icon={faHeart}
            style={{ color: `${isLike ? "#ff7f50" : "#c5c5c5"}` }}
            onClick={onHeartClick}
          />
          <span className="bold-span" id="like-list" onClick={onlikeListClick}>
            좋아요
          </span>
          ...
          {likeModal && (
            <>
              ...
              <Modal
                isOpenModal={likeModal}
                setIsOpenModal={setLikeModal}
                children={
                  <>
          ...
          <span className="bold-span">댓글</span>
          <span>{postObj.commentCount}명</span>
        </div>
      ) : (
        <div className="heart-comment-wrapper">
          <FontAwesomeIcon
